...
/***************************************************************************
 AutomatedReportingTool
                                 A QGIS plugin
 This plugin aims to perform spatial analysis and automate map production for the EIA process.
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
                              -------------------
        begin                : 2019-09-02
        git sha              : $Format:%H$
        copyright            : (C) 2019 by Rui Jiang
        email                : jiangrui1123@outlook.com
 ***************************************************************************/
...
from .automated_reporting_tool_dialog import AutomatedReportingToolDialog
import os.path
import os
import psycopg2
import processing
...
class AutomatedReportingTool:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'AutomatedReportingTool_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Automated Reporting Tool for EIA Mapping Outputs')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('AutomatedReportingTool', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/automated_reporting_tool/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Automated Reporting Tool for the EIA process'),
            callback=self.run,
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&Automated Reporting Tool for EIA Mapping Outputs'),
                action)
    ...
    def load_env_shp(self):
        try:
            ...
            if layer_name not in list_of_layers:
                ...
                QgsProject.instance().layerTreeRoot().findLayer(env_layer_existing.id()).setItemVisibilityChecked(
                    True)
                canvas = self.iface.mapCanvas()
    ...
    def display_buff_on_maps(self):
        ...
        try:
            ...
            writer = QgsVectorFileWriter(outFn, 'UTF-8', fields, QgsWkbTypes.Polygon, layer.sourceCrs(),
                                         'ESRI Shapefile')
            if unit == "metre":
        ...
        if len(analysis_result) == 0:
            ...
            for result in analysis_result:
                ...
                self.dlg.textBrowser_3.append(
                    str({'Dev Site': result[0], 'Env site': result[1], 'Method': result[2],
                         'Result': str(result[3])}))
        conn.commit()
    ...
    def search_for_layers(self):
        ...
        cur = conn.cursor()
        cur.execute(
            """select layer_name from dt.layer where layer_name ~* '""" + keyword + """' order by layer_name;""")
        layers = cur.fetchall()
        list_of_all_layers = []
        for layer in layers:
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = AutomatedReportingToolDialog()
